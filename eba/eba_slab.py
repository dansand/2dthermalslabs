...
from unsupported_dan.interfaces import markerLine2D
...
dp = edict({#Main physical paramters
           ...
           'Adf':1.25*3e-11, #pre-exp factor for diffusion creep
           ...
           'np':20.5, #Peierls creep stress exponent 
           ...
           #Rheology - upper /lower mantle transition
           ...
           'CRUSTVISCUTOFF':275.*1e3, #Deeper than this, crust material rheology reverts to mantle rheology
           ...
           'crustViscEndWidth':25e3, #distance over which to phase out crust weakness
           'depthViscReduction':200e3,
           'tempViscReduction': 1400
...
#0.3*dp.deltaTa + dp.TS, 0.4*dp.deltaTa + dp.TS
...
md = edict({'refineMesh':False,
            ...
            'viscReduceFac':0.1,
            'stressMaxReduceFac':0.03,
            'slabViscWeakening':False,
            'slabPlasWeakening':False
...
if not checkpointLoad:
    ...
    sf = edict({'stress':dp.LS**2/(dp.k*dp.eta0),
            ...
            'Apr':9.25716664691273e+40 #same form as Ads, but ndp.np =20.5, dp.eta0=1e20 (hardcoded because numbers are too big)
    ...
    ndp = edict({
             ...
             'crustViscEndWidth':dp.crustViscEndWidth/dp.LS,
             'tempViscReduction':(dp.tempViscReduction - dp.TS)/dp.deltaTa,
             'depthViscReduction':dp.depthViscReduction/dp.LS
            })
...
#ndp.Di
#dp.deltaTa
#dp.Ads*sf.Ads,dp.Adf*sf.Adf,dp.Apr*sf.Apr
...
filesMy = 1.0e6 #dimensional time intervale to write files
...
_temps[0] = ndp.TP #need to work in dimensionless absolute temps, i.e. ndp.TP, not ndp.TPP
i = 1
for delta_z in dz:
...
surfaceCoords =np.ones((Xres*2, 2))*1.
surfaceCoords[:,0] = np.linspace(MINX, MAXX, surfaceCoords.shape[0])
...
surfaceAge = surface.swarm.add_variable( dataType="float", count=1 )
...
#md.slabViscWeakening = True
#md.slabPlasWeakening= True


# In[74]:

##############
#Set up slab viscosity and yield Stress max reduction - 
##############


#dtdtL = (1. - ndp.TPP)  #Linearised temp grad across mantle
#depthViscReduction = 100e3/dp.LS
#tempViscReduction = 0.9*ndp.TPP + dtdtL * depthFn




if md.slabViscWeakening:

    slabViscReduceFn  = fn.branching.conditional([( operator.and_(temperatureField < ndp.tempViscReduction, depthFn > ndp.depthViscReduction)
    , md.viscReduceFac),
                                                         (True, fn.misc.constant(1.))])

else:
    slabViscReduceFn = fn.misc.constant(1.)
                                                  
                                                  

if md.slabPlasWeakening:

    slabStressMaxReduceFn  = fn.branching.conditional([( operator.and_(temperatureField < ndp.tempViscReduction, depthFn > ndp.depthViscReduction)
    , md.stressMaxReduceFac),
                                                         (True, fn.misc.constant(1.))])

else:
    slabStressMaxReduceFn = fn.misc.constant(1.)
    


# In[75]:

#md.slabPlasWeakening, md.slabViscWeakening
...
peierls = pefac*srfac*fn.math.exp( ((ndp.Eps + (depthFn*ndp.Wps))/(ndp.np*(temperatureField+ 0.3*depthFn + ndp.TS))))


##Define the mantle Plasticity
ys =  ndp.cm + (depthFn*ndp.fcmd)
ysf = fn.misc.min(ys, slabStressMaxReduceFn*ndp.ysMax)
yielding = ysf/(2.*(strainRate_2ndInvariant)) 

##Crust rheology
crustys =  ndp.cc + (depthFn*ndp.fccd)
crustysf = fn.misc.min(crustys, slabStressMaxReduceFn*ndp.ysMax)
...
crustyielding0 = crustysf/(2.*(strainRate_2ndInvariant))
ctanhFn = 0.5*(fn.math.tanh((depthFn - ndp.CRUSTVISCUTOFF)/(ndp.crustViscEndWidth))) + 0.5
crustyielding = crustyielding0*(1. - ctanhFn) + ctanhFn*yielding
viscmaxCrustFn = ndp.eta_max_crust*(1. - ctanhFn) + ctanhFn*ndp.eta_max


# In[95]:

#Take a look at the difference between tru adiabatic profile and linearised profile
#%pylab inline

#dp.dTa = (dp.a*dp.g*(dp.TP))/dp.Cp #adibatic gradient, at Tp
#print(dp.dTa/(dp.deltaTa/dp.LS))

#testTempFn = 0.378*depthFn + ndp.TPP
#lmdiffusion1 = (1./ndp.Alm ) *fn.math.exp( ((ndp.Elm + (depthFn*ndp.Wlm))/((temperatureField+ ndp.TS)))) 
#lmdiffusion2 = (1./ndp.Alm ) *fn.math.exp( ((ndp.Elm + (depthFn*ndp.Wlm))/((testTempFn+ ndp.TS)))) 

#finalviscosityFn1  = (viscDepthWeightFn*uppermantleviscosityFn) + (1. - viscDepthWeightFn)*(lmdiffusion*ndp.low_mantle_visc_fac)
#finalviscosityFn2  = (viscDepthWeightFn*uppermantleviscosityFn) + (1. - viscDepthWeightFn)*(lmdiffusion2*ndp.low_mantle_visc_fac)

#%pylab inline

#fig, ax = plt.subplots()
#ax.scatter(finalviscosityFn1.evaluate(mesh.data[iWalls.data])[:,0], mesh.data[iWalls.data][:,1] )
#ax.scatter(finalviscosityFn2.evaluate(mesh.data[iWalls.data])[:,0], mesh.data[iWalls.data][:,1] )
...
if md.viscCombine == 'harmonic':
    ...
    crustviscosityFn = safe_visc(1./crust_denom, viscmin=ndp.eta_min_crust, viscmax=viscmaxCrustFn)
...
if md.viscCombine == 'min':
    ...
    crustviscosityFn = safe_visc(fn.misc.min(finalviscosityFn, crustyielding), viscmin=ndp.eta_min, viscmax=viscmaxCrustFn)
    #Crust viscosity only active above between CRUSTVISCUTOFF and MANTLETOCRUST
    finalcrustviscosityFn  = crustviscosityFn
...
viscMin = ndp.eta_max*slabViscReduceFn
...
#some setup for contours on Temp


isoInt = 1./(dp.deltaTa/400.) #isotherm interval
tempCorr = temperatureField - (1  - ndp.TPP)*depthFn


# In[239]:

#fig3 = glucifer.Figure(figsize=(300*np.round(md.aspectRatio,2),300))
#fig3.append( glucifer.objects.Points(gSwarm,tempCorr, fn_mask=vizVariable))
#fig3.append( glucifer.objects.Contours(mesh,tempCorr, colours='black', interval=isoInt, limits=(0.1, 4*isoInt)))
#fig3.append( glucifer.objects.Contours(mesh,temperatureField, colours='black', interval=isoInt, limits=(0.1, 4*isoInt)))
...
if figures == 'gldb':
    ...
    figTemp.append( glucifer.objects.Points(gSwarm,temperatureField, valueRange =[0.,0.7], fn_mask=vizVariable))
    figTemp.append( glucifer.objects.Contours(mesh,tempCorr, colours='black', interval=isoInt, limits=(0.1, 4*isoInt)))
    figTemp.append( glucifer.objects.Contours(mesh,temperatureField, colours='black', interval=isoInt, limits=(0.1, 4*isoInt)))
    ...
    figVisc.append( glucifer.objects.Contours(mesh,tempCorr, colours='black', interval=isoInt, limits=(0.1, 4*isoInt)))
    figVisc.append( glucifer.objects.Contours(mesh,temperatureField, colours='black', interval=isoInt, limits=(0.1, 4*isoInt)))
    ...
    figMech.append( glucifer.objects.Contours(mesh,tempCorr, colours='black', interval=isoInt, limits=(0.1, 4*isoInt)))
    figMech.append( glucifer.objects.Contours(mesh,temperatureField, colours='black', interval=isoInt, limits=(0.1, 4*isoInt)))
    ...
    figSr= glucifer.Figure(store5, figsize=(300*np.round(md.aspectRatio,2),300))
    figSr.append( glucifer.objects.Points(gSwarm,strainRate_2ndInvariant, logScale=True, fn_mask=vizVariable))
...
while realtime < 1.:
    ...
    if files_this_step:
        ...
        #Now the vertical stress sub comps at the surface, 
        ...
        #Now the age at the surface, 
        fnametemp = "surfaceAge" + "_" + str(step) + '.h5'
        fullpath = os.path.join(outputPath + "files/" + fnametemp)
        try:
            surfaceAge.data[:,0] =    np.average(ageVariable.data[ix1][:,:,0], weights=weights1, axis=1)
        except:
            pass
        surfaceAge.save(fullpath)
    ...
    if files_this_step: 
        ...
        if figures == 'gldb':
            ...
            figSr.save( fullpath + "Sr"    + str(step).zfill(4))
